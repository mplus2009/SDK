...
class _NotificarScreenState extends State<NotificarScreen> {
  ...
  Future<void> _cargarCatalogos() async {
    final meritos = await ApiService.getCatalogo('meritos');
    final demeritos = await ApiService.getCatalogo('demeritos');
...
Future<void> _buscarEstudiantes(String query) async {
  ...
  final resultados = await ApiService.buscarEstudiantes(query);
  if (!mounted) return;
...
void _seleccionarActividad(dynamic item) {
  setState(() {
    _actividadSeleccionada = item;
    _resultadosActividad = [];
  });

  if (_tipoActual == 'merito') {
    _cantidadController.text = '${item['meritos'] ?? 1}';
  } else {
    _actualizarSliders();
  }
}
...
// ============================================
// ACTUALIZAR SLIDERS SEGUN GRADO
// ============================================
void _actualizarSliders() {
  ...
  if (hay10mo && _actividadSeleccionada['demeritos_10mo'] != null) {
    ...
    setState(() {
      _sliderMin10mo = rango[0];
      _sliderMax10mo = rango[1];
      _slider10mo = rango[0].toDouble();
    });
  }
  ...
  if (hay11_12 && _actividadSeleccionada['demeritos_11_12'] != null) {
    ...
    setState(() {
      _sliderMin11_12 = rango[0];
      _sliderMax11_12 = rango[1];
      _slider11_12 = rango[0].toDouble();
    });
  }
}
...
Map<String, List<String>> _obtenerGruposPorGrado() {
  ...
  for (final d in _destinatarios) {
    ...
    if (grado == '10mo') {
      grupos['10mo']!.add(d['nombre']);
    } else {
      grupos['11no']!.add("${d['nombre']} ($grado)");
    }
  }
  return grupos;
}
...
void _agregarActividad() {
  if (_actividadSeleccionada == null) {
    _showSnackBar('Selecciona una actividad', Colors.orange);
    return;
  }
  ...
  setState(() {
    _actividadesAgregadas.add({
      'tipo': _tipoActual,
      'categoria': _categoriaFiltro ?? _actividadSeleccionada['categoria'],
...
Future<void> _enviarNotificacion() async {
  ...
  final data = {
    ...
    'id_star': _idStar,
    'tipo_notificador': _tipoNotificador,
    'cargo_notificador': _cargoNotificador,
  };
  ...
  if (response['success'] == true) {
    _showSnackBar('Notificacion enviada correctamente', Colors.green);
    setState(() {
      _destinatarios.clear();
      _actividadesAgregadas.clear();
      _actividadSeleccionada = null;
    });

    if (_usandoCuentaTemporal) {
      setState(() {
        _usandoCuentaTemporal = false;
        _idStar = ApiService.usuario?.id ?? 0;
        _tipoNotificador = 'cuenta';
        _cargoNotificador = ApiService.usuario?.cargo ?? '';
        _notificadorNombre = ApiService.usuario?.nombreCompleto ?? '';
        _notificadorCargo = ApiService.usuario?.cargo ?? '';
      });
    }
  } else {
...
Future<void> _verificarCuentaTemporal() async {
  ...
  final response = await ApiService.verificarNotificador(nombre, password);
  ...
  if (response['success'] == true) {
    setState(() {
      _idStar = response['id'];
      _tipoNotificador = 'temporal';
      _cargoNotificador = response['cargo'];
      _notificadorNombre = response['nombre'];
      _notificadorCargo = response['cargo'] ?? 'Temporal';
      _usandoCuentaTemporal = true;
      _tempNombreController.clear();
...
void _mostrarConfirmacion() {
  showModalBottomSheet(
    ...
    builder: (context) => _buildConfirmacionModal(),
  );
}

Widget _buildConfirmacionModal() {
  final grupos = _obtenerGruposPorGrado();
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const Text('Confirmar Notificacion', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: Color(0xFF1E3C72))),
        const SizedBox(height: 20),
        ...
        ...(_actividadesAgregadas.map((a) {
          ...
          if (hayAmbos && a['valor10mo'] != null && a['valor11_12'] != null) {
            valor = '10mo: -${a['valor10mo']} | 11/12: -${a['valor11_12']}';
          }
          return Container(
...
Widget build(BuildContext context) {
  ...
  for (final item in _catalogoActual) {
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_destinatarios.isEmpty)
                        Container(
                          padding: const EdgeInsets.all(12),
                          decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFCBD5E0), style: BorderStyle.solid)),
                        ...
                        Wrap(
                          spacing: 8,
                          runSpacing: 8,
                          children: _destinatarios.asMap().entries.map((entry) {
                            ...
                            return Chip(
                              ...
                              onDeleted: () => setState(() => _destinatarios.removeAt(index)),
                            );
                          }).toList(),
                        ),
                      const SizedBox(height: 15),
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'Buscar por CI, ID o nombre...',
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(
                                ...
                                onTap: () => _agregarDestinatario(est),
                              );
                            },
                          ),
                        ),
                        ...
                        child: OutlinedButton.icon(
                          onPressed: () async {
                            ...
                            if (result != null && mounted) {
                              setState(() => _destinatarios.addAll(result as List<Map<String, dynamic>>));
                            }
                          },
                          ...
                          style: OutlinedButton.styleFrom(
                            ...
                            side: const BorderSide(color: Color(0xFF1E3C72), style: BorderStyle.solid),
                ...
                const SizedBox(height: 20),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (_actividadesAgregadas.isEmpty)
                          Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFCBD5E0), style: BorderStyle.solid)), child: const Center(child: Text('No hay actividades agregadas', style: TextStyle(color: Color(0xFF888888)))))
                        else
                          ...(_actividadesAgregadas.asMap().entries.map((entry) {
                            final index = entry.key;
                            final act = entry.value;
                            final bg = act['tipo'] == 'merito' ? const Color(0xFFD1FAE5) : const Color(0xFFFEE2E2);
                            final border = act['tipo'] == 'merito' ? const Color(0xFF10B981) : const Color(0xFFEF4444);
                            final color = act['tipo'] == 'merito' ? const Color(0xFF065F46) : const Color(0xFF991B1B);
                            return Container(
                              margin: const EdgeInsets.only(bottom: 10),
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(12), border: Border(left: BorderSide(color: border, width: 4))),
                              child: Row(children: [
                                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Text(act['nombre'], style: TextStyle(fontWeight: FontWeight.w600, color: color)),
                                ...
                                GestureDetector(onTap: () => setState(() => _actividadesAgregadas.removeAt(index)), child: const Icon(Icons.delete, color: Color(0xFFEF4444))),
                              ]),
                            );
                          })),
                        const SizedBox(height: 15),
                        ...
                        TextField(
                          decoration: InputDecoration(hintText: 'Buscar merito o demerito...', prefixIcon: const Icon(Icons.search, color: Color(0xFF999999)), border: OutlineInputBorder(borderRadius: BorderRadius.circular(14))),
                          onChanged: _buscarActividades,
                        ),
                        if (_resultadosActividad.isNotEmpty)
                          Container(
                            ...
                            child: ListView.builder(
                              shrinkWrap: true,
                              itemCount: _resultadosActividad.length,
                              itemBuilder: (context, index) {
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items: [const DropdownMenuItem(value: null, child: Text('Todas')), ...categorias.map((c) => DropdownMenuItem(value: c, child: Text(c)))],
                        ...
                        if (_tipoActual == 'demerito' && hay10mo && _actividadSeleccionada != null)
                          Container(padding: const EdgeInsets.all(16), margin: const EdgeInsets.only(bottom: 15), decoration: BoxDecoration(color: const Color(0xFFFFF3CD), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFFFC107), width: 2)), child: Column(children: [
                            ...
                            Text('Estudiantes: ${grupos['10mo']!.join(', ')}', style: const TextStyle(fontSize: 12, color: Color(0xFF856404))),
                        ...
                        if (_tipoActual == 'demerito' && hay11_12 && _actividadSeleccionada != null)
                          Container(padding: const EdgeInsets.all(16), margin: const EdgeInsets.only(bottom: 15), decoration: BoxDecoration(color: const Color(0xFFFFF3CD), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFFFC107), width: 2)), child: Column(children: [
                            ...
                            Text('Estudiantes: ${grupos['11no']!.join(', ')}', style: const TextStyle(fontSize: 12, color: Color(0xFF856404))),
                  ...
                  // Quien Notifica
                  Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 15)]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('Quien Notifica', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Color(0xFF1E3C72))),
                    const SizedBox(height: 15),
                    Row(children: [
                      const Icon(Icons.account_circle, size: 42, color: Color(0xFF667EEA)),
                      const SizedBox(width: 14),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(_usandoCuentaTemporal ? 'Cuenta Temporal' : 'Yo (Cuenta actual)', style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF1E293B))),
                        Text(_notificadorNombre, style: const TextStyle(color: Color(0xFF64748B))),
                        Text(_notificadorCargo, style: const TextStyle(fontSize: 12, color: Color(0xFF64748B))),
                      ])),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          TextField(controller: _tempPasswordController, obscureText: true, decoration: const InputDecoration(labelText: 'Contrasena')),
        ]),
        actions: [
