...
class _EscanerScreenState extends State<EscanerScreen> {
  ...
  void _procesarQR(String? code) {
    ...
    try {
      ...
      if (usuario != null && usuario['nombre'] != null && usuario['apellidos'] != null) {
        ...
        if (!existe) {
          setState(() {
            _escaneados.add({'id': id, 'nombre': '${usuario!['nombre']} ${usuario['apellidos']}', 'ci': '${usuario['ci'] ?? ''}', 'grado': '${usuario['grado'] ?? '10mo'}', 'cargo': '${usuario['cargo'] ?? 'estudiante'}'});
          });
  ...
  Map<String, dynamic>? _decodificarQR(String code) {
    try {
      ...
      if (data is Map<String, dynamic>) {
        return {'id': '${data['id'] ?? ''}', 'nombre': '${data['nombre'] ?? ''}', 'apellidos': '${data['apellidos'] ?? ''}', 'ci': '${data['ci'] ?? ''}', 'grado': '${data['grado'] ?? '10mo'}', 'cargo': '${data['cargo'] ?? 'estudiante'}'};
      }
    } catch (e) {}
    try {
  ...
  void _showSnackBar(String message, Color color) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message), backgroundColor: color, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), margin: const EdgeInsets.all(16), duration: const Duration(seconds: 2)));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [const Text('Escanear QR'), const Spacer(), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: const Color(0xFF10B981), borderRadius: BorderRadius.circular(20)), child: Text('${_escaneados.length}', style: const TextStyle(fontWeight: FontWeight.w600)))]),
        leading: IconButton(icon: const Icon(Icons.arrow_back), onPressed: () => Navigator.pop(context, _escaneados)),
      ),
      body: Stack(
        children: [
          MobileScanner(controller: _controller, onDetect: (capture) { final barcodes = capture.barcodes; if (barcodes.isNotEmpty) { final barcode = barcodes.first; if (barcode.rawValue != null) _procesarQR(barcode.rawValue); } }),
          Center(
            child: Container(
              width: 250, height: 250,
              decoration: BoxDecoration(border: Border.all(color: const Color(0xFF10B981), width: 3), borderRadius: BorderRadius.circular(20)),
              child: Stack(
                children: [
          ...
          if (_escaneados.isNotEmpty)
            Positioned(
              ...
              child: Container(
                ...
                decoration: const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.vertical(top: Radius.circular(24)), boxShadow: [BoxShadow(color: Colors.black26, blurRadius: 20)]),
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Row(children: [const Icon(Icons.check_circle, color: Color(0xFF10B981)), const SizedBox(width: 8), Text('Estudiantes escaneados (${_escaneados.length})', style: const TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF1E3C72), fontSize: 16))]),
                    const SizedBox(height: 10),
                    SizedBox(
                      height: 120,
                      child: ListView.builder(
                        shrinkWrap: true, itemCount: _escaneados.length,
                        itemBuilder: (context, index) {
                          ...
                          return Container(
                            ...
                            child: ListTile(
                              dense: true,
                              leading: const CircleAvatar(backgroundColor: Color(0xFF667EEA), child: Icon(Icons.person, color: Colors.white, size: 20)),
                              title: Text('${e['nombre']} (${e['ci']})', style: const TextStyle(fontSize: 14, color: Color(0xFF1E3C72))),
                              subtitle: Text(e['grado'] ?? '', style: const TextStyle(fontSize: 12)),
                              trailing: IconButton(icon: const Icon(Icons.close, color: Colors.red, size: 20), onPressed: () => setState(() => _escaneados.removeAt(index))),
                            ),
                          );
                        },
                      ),
